...
struct bigint {
    ...
    friend bigint exgcd(bigint a, bigint b, bigint &x, bigint &y) {
        if (b==0) {
            x = 1, y = 0;
            return a;
        }
        bigint g = exgcd(b, a % b, y, x);
        y -= a / b * x;
        return g;
    }
    friend bigint phi(bigint x){
        bigint res = x;
        for (bigint i = 2; i <= x / i; i +=1 )
            if (x % i == 0){
                res = res / i * (i - 1);
                while (x % i == 0) x /= i;
            }
    ...
    friend bigint pow_mod(bigint a, bigint n, bigint mod){
        bigint res = 1;
        while (n!=0){
