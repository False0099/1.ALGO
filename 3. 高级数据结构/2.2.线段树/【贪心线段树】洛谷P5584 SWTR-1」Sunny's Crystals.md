...
有一天，小 $\mathrm{A}$ 来到了小 $\mathrm{S}$ 家，让小 $\mathrm{S}$ 把他的水晶排成一个序列，并且摧毁所有价值为 $w$ 的水晶。
...
接下来一行 $m$ 个数，第 $i$ 个数表示第 $i$ 次摧毁时被摧毁的水晶的**初始位置**是什么，**用空格隔开**。
...
## 题解
本题我们考虑贪心，我们每一次如果有能够摧毁的，我们就去摧毁我们的**最靠后的哪一个位置**，如果我们没有能够摧毁的，我们就去**摧毁我们的第一个位置**。

于是，我们可以考虑先去预处理出下面的内容：
1. 我们的每一个 W 到距离他最近的可以摧毁的位置的距离是多少（$o(n)$）
